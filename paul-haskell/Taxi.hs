...
changes s = streamFilterAcc (value $ head s) (\h acc-> if (h==acc) then acc else h) (\h acc->(h/=acc)) (tail s)
...
profitableCells s = changes 
                  ...
                  $ joinStreamsW processedStream (slidingTime 900) processedStream (slidingTime 1800)
                                (\a b->profitability (emptyTaxisPerCell b)(cellProfit a))
